...
def gh_get_user_starred_repositories(github_username):
    ...
    for rep_ind in range(len(user_starred_reps)):
        ...
        user_starred_reps[rep_ind]["owner"] = user_starred_reps[rep_ind]["owner"][
            "login"
        ]
    ...
    while has_next_stargazer:
        ...
        while not is_response_ok:
            if github_query_vars["stargazers_batch_size"] == 0:
                break
            stargazers_batch_response = requests.post(
                url,
                json={"query": github_query_template.render(github_query_vars)},
                headers=head,
            )
            ...
            if stargazers_batch_response.status_code == 200:
                ...
                logger.debug(
                    "gh_get_stargazers: batch size decresing from "
                    f"{str(github_query_vars['stargazers_batch_size'])} "
                    f"to {str(int(github_query_vars['stargazers_batch_size'] / 2))}"
                )
                github_query_vars["stargazers_batch_size"] = int(
                    github_query_vars["stargazers_batch_size"] / 2
                )
        ...
        stargazers_batch = stargazers_batch_response.json()["data"]["repository"][
            "stargazers"
        ]
        stargazers_login_batch = [
            stargazer["node"]["login"] for stargazer in stargazers_batch["edges"]
        ]
        stargazers_login.extend(stargazers_login_batch)
        has_next_stargazer = stargazers_batch["pageInfo"]["hasNextPage"]
        github_query_vars["stargazers_cursor"] = (
            'after: "' + stargazers_batch["pageInfo"]["endCursor"] + '"'
        )

    logger.debug(
        f"Retrieved stargazers for repository {repository.name}, "
...
def gh_get_starred_reps_list(
    users_login, users_batch_size, github_query_template, head
):
    logger.debug(
    ...
    while len(processing_users) > 0:
        ...
        while not is_response_ok:
            if github_query_vars["reps_batch_size"] == 0:
                break
            starred_reps_batch_response = requests.post(
                url,
                json={"query": github_query_template.render(github_query_vars)},
                headers=head,
            )
            ...
            if starred_reps_batch_response.status_code == 200:
                ...
                logger.debug(
                    "gh_get_starred_reps_list: batch size decresing from "
                    f"{str(github_query_vars['reps_batch_size'])} "
                    f"to {str(int(github_query_vars['reps_batch_size'] / 2))}"
                )
                github_query_vars["reps_batch_size"] = int(
                    github_query_vars["reps_batch_size"] / 2
                )

        if not is_response_ok:
            raise GhApiException(
                "Get starred repositories: github api error for "
...
def gh_get_starred_repositories_of_stargazers(repositories, users_batch_size):
    ...
    for repository in repositories.all():
        stargazers_login_rep = gh_get_stargazers(
            repository, github_stargazers_template, head
        )
    ...
    starred_reps_by_user = gh_get_starred_reps_list(
        stargazers_login, users_batch_size, github_starred_reps_batch_template, head
    )
